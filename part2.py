...
class NetworkCnn(tnn.Module):
    ...
    def __init__(self):
        ...
        self.max_pool = tnn.AdaptiveAvgPool1d(1)
        self.ln = tnn.Linear(50, 1)
    ...
    def forward(self, input, length):
        ...
        x = self.pool(tnn.functional.relu(self.conv(input)))
        x = self.pool(tnn.functional.relu(self.conv(x)))
        x = tnn.functional.relu(self.conv(x))
        x = self.max_pool(x)
...
def measures(outputs, labels):
    ...
    """
    tp = tn = fp = fn = 0
    for o_it, l_it in zip(outputs, labels):
        if o_it > 0 and l_it == 1:
            tp += 1
        elif o_it < 0 and l_it == 0:
            tn += 1
        elif o_it < 0 and l_it == 0:
            fn += 1
        elif o_it > 0 and l_it == 1:
            fp += 1
